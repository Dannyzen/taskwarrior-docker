...
RUN curl -O http://taskwarrior.org/download/taskd-1.1.0.tar.gz
...
RUN curl -O http://taskwarrior.org/download/task-2.4.4.tar.gz
RUN tar xzf task-2.4.4.tar.gz
RUN cd task-2.4.4
WORKDIR /task-2.4.4
RUN cmake .
RUN make
...
ADD ./init_folks.sh /init_folks.sh
